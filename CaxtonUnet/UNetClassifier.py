...
DATASET_DIR = '/Users/lydierosenkrancova/Desktop/SU2/2024-final-hlina-rules/SU2_cambridge_data_FirstAttempt_multifiles_load/caxton_dataset_small'
...
image_train, image_temp, labels_train, labels_temp, nozzle_coords_train, nozzle_coords_temp = train_test_split(
    all_image_paths, all_labels, all_nozzle_coords, test_size=0.995, random_state=42
)

image_val, image_tempor, labels_val, labels_tempor, nozzle_coords_val, nozzle_coords_tempor = train_test_split(
    image_temp, labels_temp, nozzle_coords_temp, test_size=0.998, random_state=42
)

image_test, image_redund, labels_test, labels_redund, nozzle_coords_test, nozzle_coords_redund = train_test_split(
    image_tempor, labels_tempor, nozzle_coords_tempor, test_size=0.998, random_state=42
)
...
class PrintAnomalyClassifier():
    ...
    def train_loop(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            avg_val_loss, val_hotend_predictions, val_hotend_labels, val_z_offset_predictions, val_z_offset_labels, val_feed_rate_predictions, val_feed_rate_labels, val_flow_rate_predictions, val_flow_rate_labels = self.validate_model(
                self.val_loader, self.loss_fn)
