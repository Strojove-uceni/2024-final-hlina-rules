...
from DataPreprocessing import CustomDataset
from tqdm import tqdm
import time
import pandas as pd
import os
from sklearn.model_selection import train_test_split
from DataPreprocessing import TransformPipeline, RandomRotate, RandomPerspectiveTransform, CenteredCrop, RandomCrop
from DataPreprocessing import HorizontalFlipColorJitter, NormalizeChannels, CalculateMeanStd
...
print("train - val - test partition")
image_train, image_temp, labels_train, labels_temp, nozzle_coords_train, nozzle_coords_temp = train_test_split(
    all_image_paths, all_labels, all_nozzle_coords, test_size=0.70, random_state=42
)

image_test, image_val, labels_test, labels_val, nozzle_coords_test, nozzle_coords_val = train_test_split(
    image_temp, labels_temp, nozzle_coords_temp, test_size=0.60, random_state=42
)

#store most of the dataset to side to enable quick debugging
"""
image_test, image_redund, labels_test, labels_redund, nozzle_coords_test, nozzle_coords_redund = train_test_split(
    image_tempor, labels_tempor, nozzle_coords_tempor, test_size=0.90, random_state=42
)
"""
...
class PrintAnomalyClassifier():
    def __init__(self):
        ...
        self.model = CustomResNet(resnet)
        ...
        print("val, test data ready")
        self.train_loader = DataLoader(self.train_dataset, batch_size=16, shuffle=True, drop_last=True)
        self.val_loader = DataLoader(self.val_dataset, batch_size=16, shuffle=True, drop_last=False)
    ...
    def test_model(self):
        try:
            if os.path.exists('short_best_resnet.pth'):
                self.model.load_state_dict(torch.load('short_best_resnet.pth'))
                print("Loaded model from 'short_best_resnet.pth'")
            elif os.path.exists('short_current_resnet.pth'):
                self.model.load_state_dict(torch.load('short_current_resnet.pth'))
                print("Loaded model from 'short_current_resnet.pth'")
            else:
                raise FileNotFoundError("Neither 'short_best_resnet.pth' nor 'short_current_resnet.pth' found.")
    ...
    def train_loop(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if avg_val_loss < best_val_loss:
                ...
                torch.save(self.model.state_dict(), 'short_best_resnet.pth')
                self.epochs_without_improvement = 0
            else:
                self.epochs_without_improvement += 1
                torch.save(self.model.state_dict(), 'short_current_resnet.pth')
    ...
    def evaluate_model(self):
        ...
        train_hotend_metrics = compute_metrics(train_hotend_labels, train_hotend_predictions)
        train_z_offset_metrics = compute_metrics(train_z_offset_labels, train_z_offset_predictions)
        train_feed_rate_metrics = compute_metrics(train_feed_rate_labels, train_feed_rate_predictions)
        train_flow_rate_metrics = compute_metrics(train_flow_rate_labels, train_flow_rate_predictions)

        val_hotend_metrics = compute_metrics(val_hotend_labels, val_hotend_predictions)
        val_z_offset_metrics = compute_metrics(val_z_offset_labels, val_z_offset_predictions)
        val_feed_rate_metrics = compute_metrics(val_feed_rate_labels, val_feed_rate_predictions)
        val_flow_rate_metrics = compute_metrics(val_flow_rate_labels, val_flow_rate_predictions)

        test_hotend_metrics = compute_metrics(test_hotend_labels, test_hotend_predictions)
        test_z_offset_metrics = compute_metrics(test_z_offset_labels, test_z_offset_predictions)
        test_feed_rate_metrics = compute_metrics(test_feed_rate_labels, test_feed_rate_predictions)
        test_flow_rate_metrics = compute_metrics(test_flow_rate_labels, test_flow_rate_predictions)

        train_predslabels = {
            'train_hotend_predictions': train_hotend_predictions,
            'train_hotend_labels': train_hotend_labels,
            'train_z_offset_predictions': train_z_offset_predictions,
            'train_z_offset_labels': train_z_offset_labels,
            'train_feed_rate_predictions': train_feed_rate_predictions,
            'train_feed_rate_labels': train_feed_rate_labels,
            'train_flow_rate_predictions': train_flow_rate_predictions,
            'train_flow_rate_labels': train_flow_rate_labels
        }

        val_predslabels = {
            'val_hotend_predictions': val_hotend_predictions,
            'val_hotend_labels': val_hotend_labels,
            'val_z_offset_predictions': val_z_offset_predictions,
            'val_z_offset_labels': val_z_offset_labels,
            'val_feed_rate_predictions': val_feed_rate_predictions,
            'val_feed_rate_labels': val_feed_rate_labels,
            'val_flow_rate_predictions': val_flow_rate_predictions,
            'val_flow_rate_labels': val_flow_rate_labels
        }

        test_predslabels = {
            'test_hotend_predictions': test_hotend_predictions,
            'test_hotend_labels': test_hotend_labels,
            'test_z_offset_predictions': test_z_offset_predictions,
            'test_z_offset_labels': test_z_offset_labels,
            'test_feed_rate_predictions': test_feed_rate_predictions,
            'test_feed_rate_labels': test_feed_rate_labels,
            'test_flow_rate_predictions': test_flow_rate_predictions,
            'test_flow_rate_labels': test_flow_rate_labels
        }
        ...
        with open('short_resnet_train_metrics.pkl', 'wb') as f:
            pickle.dump(train_metrics, f)

        with open('short_resnet_val_metrics.pkl', 'wb') as f:
            pickle.dump(val_metrics, f)

        with open('short_resnet_test_metrics.pkl', 'wb') as f:
            pickle.dump(test_metrics, f)

        with open('short_resnet_train_labels.pkl', 'wb') as f:
            pickle.dump(train_predslabels, f)

        with open('short_resnet_val_labels.pkl', 'wb') as f:
            pickle.dump(val_predslabels, f)

        with open('short_resnet_test_labels.pkl', 'wb') as f:
            pickle.dump(test_predslabels, f)
    ...
    def run(self):
        ...
        with open('short_resnet_time_elapsed.pkl', 'wb') as f:
