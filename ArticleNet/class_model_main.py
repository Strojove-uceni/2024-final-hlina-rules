...
print(len(all_image_paths), len(all_labels), len(all_nozzle_coords))
...
class PrintAnomalyClassifier():
    ...
    def train_loop(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if avg_val_loss < best_val_loss:
                ...
                torch.save(self.model.state_dict(), 'short_best_rosnet.pth')
                self.epochs_without_improvement = 0
            else:
                self.epochs_without_improvement += 1
                torch.save(self.model.state_dict(), 'short_current_rosnet.pth')
    ...
    def test_model(self):
        try:
            if os.path.exists('short_best_rosnet.pth'):
                self.model.load_state_dict(torch.load('short_best_rosnet.pth'))
                print("Loaded model from 'short_best_rosnet.pth'")
            elif os.path.exists('short_current_rosnet.pth'):
                self.model.load_state_dict(torch.load('short_current_rosnet.pth'))
                print("Loaded model from 'short_current_unet.pth'")
            else:
                raise FileNotFoundError("Neither 'short_best_unet.pth' nor 'short_current_unet.pth' found.")
    ...
    def evaluate_model(self):
        ...
        with open('short_rosnet_train_metrics.pkl', 'wb') as f:
            pickle.dump(train_metrics, f)

        with open('short_rosnet_val_metrics.pkl', 'wb') as f:
            pickle.dump(val_metrics, f)

        with open('short_rosnet_test_metrics.pkl', 'wb') as f:
            pickle.dump(test_metrics, f)

        with open('short_rosnet_train_preds.pkl', 'wb') as f:
            pickle.dump(train_predslabels, f)

        with open('short_rosnet_val_preds.pkl', 'wb') as f:
            pickle.dump(val_predslabels, f)

        with open('short_rosnet_test_preds.pkl', 'wb') as f:
    ...
    def run(self):
        ...
        with open('short_time_elapsed.pkl', 'wb') as f:
